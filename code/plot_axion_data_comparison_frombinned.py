...
def read_HEPdata_SM():
    data = csv_reader('../../sm_nnlo/data/HEPData-ins1901295-v1-parton_abs_ttm.csv')
    cms_data  = []
    for item in data[9:24]:
        cms_data.append(float(item[3]))

    covdata = csv_reader('../../sm_nnlo/data/HEPData-ins1901295-v1-parton_abs_ttm_covariance.csv')
...
def axion_signal(ct, bins, BR, kfac, kfac_unc):
    ...
    '''
    axion_lin = np.divide(BR*np.loadtxt('../saved_axion_hist_frombinned/mtt_ct1p0_nnpdf4p0_alphaS_linear.txt',usecols=(2,),dtype=float),bins_width)
    axion_quad = np.divide(BR*np.loadtxt('../saved_axion_hist_frombinned/mtt_ct1p0_nnpdf4p0_alphaS_quad.txt',usecols=(2,),dtype=float),bins_width)
    sm = np.divide(BR*np.loadtxt('../../sm_lo/mtt_SM_ttbar_nnpdf4p0.txt',usecols=(0,),dtype=float), bins_width)
...
if __name__=='__main__':
    ...
    ctvals = [0.0, 4.0, 10.0, 20.0]
    BR = 0.2877
    kfac = np.loadtxt('../../sm_kfac_notebooks/kfac_nnlo_lo_highstats.txt', dtype=float, usecols=(0,))
    kfac_unc = np.loadtxt('../../sm_kfac_notebooks/kfac_nnlo_lo_highstats.txt', dtype=float, usecols=(1,))
    ...
    #Plot data
    ax.step(bins_cms, cms_data_plot, where='post',label='CMS data (fig 19 (a) of 2108.02803) ', color=colors[0])
    ...
    ax.set_ylabel(r'$\frac{d \sigma}{d m_{t \bar{t}}}$')


    ax_ratio.set_xlabel(r'$m_{t \bar{t}}$ [GeV]')
    ax_ratio.set_ylabel('Ratio to data')
    ax_ratio.hlines(xmin=355, xmax=4000, y=1.0,color='black')
    
    for ind in range(len(ctvals)):
        axion_data = axion_signal(ctvals[ind], bins_cms, BR, kfac, kfac_unc)
        ...
        ax.step(bins_cms, axion_data_plot, where='post',label=r'SM + ALP, $c_{t} = $'+str(ctvals[ind]), color=colors[ind+1])
