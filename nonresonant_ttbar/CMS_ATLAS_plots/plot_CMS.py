...
def axion_signal(ct, bins, BR, kfac, kfac_unc):
    ...
    and uncertainties
    '''
    axion_lin = np.divide(BR*np.loadtxt('./Madgraph/mtt_ct1p0_nnpdf4p0_alphaS_linear.txt',usecols=(2,),dtype=float),bins_width)
    axion_quad = np.divide(BR*np.loadtxt('./Madgraph/mtt_ct1p0_nnpdf4p0_alphaS_quad.txt',usecols=(2,),dtype=float),bins_width)
    sm = np.divide(BR*np.loadtxt('./Madgraph/mtt_SM_ttbar_nnpdf4p0.txt',usecols=(0,),dtype=float), bins_width)
...
if __name__=='__main__':
    ...
    BR = 0.2877
    kfac = np.loadtxt('./kfactors/CMS_kfac_nnlo_lo_highstats.txt', dtype=float, usecols=(0,))
    kfac_unc = np.loadtxt('./kfactors/CMS_kfac_nnlo_lo_highstats.txt', dtype=float, usecols=(1,))
